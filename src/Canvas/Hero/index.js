...
import {MathUtils, Vector2, TextureLoader, NearestFilter, LinearMipMapLinearFilter } from 'three';
import {getBoundPosition} from '../utils';
import {useHeroStore, useMouseStore} from '../../useStore';
...
export function Hero() {
  ...
  useFrame(({clock}) => {
    ...
    if(useHeroStore.getState().isActive) {
      const mouse = useMouseStore.getState().mouse;

      uniforms.uMouse.value.x = MathUtils.lerp(uniforms.uMouse.value.x, mouse.x, 0.05);
      uniforms.uMouse.value.y = MathUtils.lerp(uniforms.uMouse.value.y, mouse.y, 0.05);
    }
